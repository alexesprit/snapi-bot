...
class NonSASL(PlugIn):
	...
	def plugin(self, owner):
		...
		if query.getTag('digest'):
			self.printf("Performing digest authentication", 'ok')
			...
			self.printf("Performing zero-k authentication", 'ok')
			hash = sha.new(sha.new(self.password).hexdigest()+token).hexdigest()
			for foo in xrange(int(seq)): hash = sha.new(hash).hexdigest()
			query.setTagData('hash', hash)
			method='0k'
		else:
			self.printf("Sequre methods unsupported,  performing plain text authentication", 'warn')
		...
		if stanzaType == TYPE_RESULT:
			...
			owner._registeredName=owner.User+'@'+owner.Server+'/'+owner.Resource
			return method
		self.printf('Authentication failed!', 'error')
	...
	def authComponent(self, owner):
		...
		while not self.handshake:
			self.printf("waiting on handshake", 'notify')
			owner.Process(1)
		owner._registeredName=self.user
